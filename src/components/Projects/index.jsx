  ...
  return (
    <>
      <Fade>
        <section
          id="work"
          className="flex flex-col w-full min-h-screen lg:flex-row card-container"
        >
            ...
            <div className="max-w-[500px] mx-auto">
              <div className="flex">
                <div
                  ...
                  onClick={() => handleProjectChange("TiLiv")}
                >
                  <p className="text-black text-[16px] font-semibold">Ti Liv</p>
                  <div
                    className={`h-[4px] w-full ${activeProject === "TiLiv" ? "card-selector-active" : "card-selector"}`}
                  ></div>
                </div>
                <div
                  className="w-[90px] cursor-pointer"
                  onClick={() => handleProjectChange("Holidaze")}
                >
                  <p className="text-center text-black text-[16px] font-semibold">
                    Holidaze
                  </p>
                  <div
                    className={`h-[4px] w-full ${activeProject === "Holidaze" ? "card-selector-active" : "card-selector"}`}
                  ></div>
                </div>
                <div
                  className="w-[85px] cursor-pointer"
                  onClick={() => handleProjectChange("BidCoin")}
                >
                  <p className="text-right text-black text-[16px] font-semibold">
                    BidCoin
                  </p>
                  <div
                    className={`h-[4px] w-full ${activeProject === "BidCoin" ? "card-selector-active" : "card-selector"}`}
                  ></div>
                </div>
              </div>
              <div className="mt-9">
          ...
          <div className="flex flex-col w-full lg:w-[50%] max-h-screen overflow-hidden">
            <Fade direction="right">
              {activeProject === "TiLiv" && (
                <img
                  src={TiLiv}
                  alt="A image of the Ti Liv site"
                  className="object-contain object-top overflow-hidden"
                />
              )}
              {activeProject === "Holidaze" && (
                <img
                  src={Holidaze}
                  alt="A image of the Holidaze Site"
                  className="object-cover object-top overflow-hidden"
                />
              )}
              {activeProject === "BidCoin" && (
                <img
                  src={BidCoin}
                  alt="A image of the BidCoin site"
                  className="object-cover object-top overflow-hidden"
                />
              )}
            </Fade>
